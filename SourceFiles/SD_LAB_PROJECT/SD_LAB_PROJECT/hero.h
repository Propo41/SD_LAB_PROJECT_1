...
void showHeroJump()
{
	if (hero_1)
		...
		iShowImage(hero.x, hero.y + hero.jump_dy, 150, 350, hero.id[heroJumpIndex]);
...
{
	if (L1Boss.direction == LEFT || L2Boss.direction == LEFT)
	{
		...
		else
		{
			...
			if ((hero.x <= L2Boss.projectileX && hero.x >= L2Boss.projectileX - 50) && (hero.y <= L2Boss.projectileY))
			{
				//cout << "BOSS LOOKING RIGHT" << endl;
				if (L2Boss.power)
				{
	...
	else
	{
		...
		else
		{
			...
			if  ( (hero.x >= L2Boss.projectileX - 10 && hero.x <= L2Boss.projectileX + 50)  && (hero.y <= L2Boss.projectileY) )
			{
				if (L2Boss.power)
				{
...
void resetHero()
{
	...
	if (hero_1)
	{
		...
		mana.variable1 = 370; // resetting mana bar
		mana.variable2 = 354;
		mana.count = 5;
	...
	if (level_2 && !bossSegment)
	{

		if (hero.direction == RIGHT)
		{
			hero.y = 180;
			hero.x -= 100; // reset from 20 pixels away
			screen.x += 40;
		}
		else
		{
