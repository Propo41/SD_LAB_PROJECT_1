...
void iKeyboard(unsigned char key)
{
	...
	//cout << "inside: " << hero.attack << " " << hero.attack2 << " " << cutscene << " " << hero.power << endl;
	 ...
	 if (!hero.attack && !hero.attack2 && !cutscene )
	{
...
void enemyAutoMotion()
{
	...
	if (bossSegment)
	{
		...
		if (level_2 && power.show)
		{

			bossAttackIndex++;
			if (bossMovingLeft && !bossMovingRight)
			{
				if (bossAttackIndex > 19)
				{
					bossAttackIndex = 19;
					bossAnimationComplete = true;
				}
				else
					bossAnimationComplete = false; // this else is used to reset the animation effect of boss after completing one attack or else
				// the for the next attacks projectile starts appearing before animation of boss shoot is complete
			}
			else if (!bossMovingLeft && bossMovingRight)
			{
				if (bossAttackIndex > 7)
				{
					bossAttackIndex = 7;
					bossAnimationComplete = true;
				}
			...
			iDelayMS(70); // this delay makes it look better at 100
...
void change()
{
	...
	if (power.show && bossAnimationComplete)  // when the boss has finally completed its shooting animation
	{

		if (reversePower) // this is true when projctile is coming back
		{ 
			if (bossTargetX > L2Boss.x)
				power.x -= 20;
			else if (L2Boss.x > bossTargetX)
				power.x += 20;
		}

		else
		{
			// this runs when the projectile is moving forward
			if (bossTargetX > L2Boss.x)
				power.x += 20;
			else if (L2Boss.x > bossTargetX)
				power.x -= 20;
	...
	if (power.distance >= 400) // when projectile have successfully travelled 400 pixel on either side its showing and swichting is changed here
	{ 
		if (reversePower)
		{
			power.show = false;
			power.distance = 0; // resetting value for next projectile
			reversePower = false;
			bossAttacking = false;
			bossAnimationComplete = false;
		}
		else
		{
			...
			power.distance = 0;
			reversePower = true; // projectile starts coming back after this
		}
